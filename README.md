...
| `ZEN_CLIENT_VERSION` | `1.18.13` | Version string used in the `User-Agent: opencode/<version> ai-sdk/provider-utils/4.0.23 runtime/bun/1.3.13` header sent on zen-direct requests (the exact wire format the official CLI sends, as captured from a real `opencode run`). |
