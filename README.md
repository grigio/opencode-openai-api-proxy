...
| `ZEN_CLIENT_VERSION` | `2.0.5` | Version string used in the `User-Agent: opencode/<channel>/<version>/opencode` header sent on zen-direct requests (the exact wire format the v2.0.5 CLI sends, e.g. `opencode/latest/2.0.5/opencode`). Pinned to `OPENCODE_CLI_TAG` for consistency. |
...
| `OPENCODE_TOOL_LOOP_LIMIT` / `TOOL_LOOP_LIMIT` | `15` | Consecutive identical tool calls before the proxy aborts with `422 Tool call loop detected` (previously `3` → now `15` to tolerate legitimate chunked writes like `write_stdin` repeated 4×). Raise to allow more retries, lower to be stricter. The error message now reports the threshold and hint. |
| `DISABLE_TOOL_LOOP_CHECK` / `TOOL_LOOP_DETECTION` | — | Set `DISABLE_TOOL_LOOP_CHECK=1` (or `TOOL_LOOP_DETECTION=false`) to disable the loop guard entirely — useful when a client legitimately polls the same tool. |
